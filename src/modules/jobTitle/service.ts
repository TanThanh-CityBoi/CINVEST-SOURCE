...
import { TJobTitle, TQueryJobTitle } from './types';

export class JobTitleService extends BaseAbstractService<JobTitleEntity> {
  constructor() {
    super(JobTitleEntity);
  }

  private readonly entityManager = connection.createEntityManager();
  async getList(query: TQueryJobTitle) {
  ...
  async create(dto: TJobTitle, em?: EntityManager) {
    const trx = em ?? this.entityManager;
    try {
      return trx.transaction((trx) => {
        return trx.save(trx.create(JobTitleEntity, { ...dto }));
      });
    } catch (error) {
  ...
  async update(id: number, dto: TJobTitle, em?: EntityManager) {
    const trx = em ?? this.entityManager;
    const data = await this.getById(id);
    if (!data) throw new grpcErrorNotFound();
    // const updateData: JobTitleEntity = Object.assign(data, dto);
    try {
      return trx.transaction((trx) => {
        return trx.save(trx.create(JobTitleEntity, { ...dto }));
      });
