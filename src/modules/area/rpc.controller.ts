...
export class rpcAreaController {
  private readonly service = new AreaService();

  async getList(
    req: grpc.ServerUnaryCall<QueryArea__Output, GetListResponse>,
  ...
  async create(
    req: grpc.ServerUnaryCall<Area__Output, GetOneResponse>,
    res: grpc.sendUnaryData<GetOneResponse>,
  ) {
    try {
      const request = convertStringToDate(req.request);
