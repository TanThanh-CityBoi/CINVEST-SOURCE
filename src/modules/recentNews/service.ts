...
import { RecentNewEntity } from './entities/recentNew.entity';
import { TQueryRecentNew, TRecentNew } from './types';

export class RecentNewService extends BaseAbstractService<RecentNewEntity> {
  constructor() {
    super(RecentNewEntity);
  }

  private readonly entityManager = connection.createEntityManager();
  async getList(query: TQueryRecentNew) {
  ...
  async create(dto: TRecentNew, em?: EntityManager) {
    const trx = em ?? this.entityManager;
    try {
      return trx.transaction((trx) => {
        return trx.save(trx.create(RecentNewEntity, { ...dto }));
      });
    } catch (error) {
  ...
  async update(id: number, dto: TRecentNew, em?: EntityManager) {
    const trx = em ?? this.entityManager;
    const data = await this.getById(id);
    if (!data) throw new grpcErrorNotFound();
    // const updateData: RecentNewEntity = Object.assign(data, dto);
    try {
      return trx.transaction((trx) => {
        return trx.save(trx.create(RecentNewEntity, { ...dto }));
      });
