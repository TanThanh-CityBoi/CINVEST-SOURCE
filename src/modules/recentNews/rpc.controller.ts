...
import { QueryRecentNew__Output } from '../../proto/types/recentNews/QueryRecentNew';
import { RecentNew__Output } from '../../proto/types/recentNews/RecentNew';
...
export class rpcRecentNewController {
  private readonly service = new RecentNewService();

  async getList(
    req: grpc.ServerUnaryCall<QueryRecentNew__Output, GetListResponse>,
  ...
  async create(
    req: grpc.ServerUnaryCall<RecentNew__Output, GetOneResponse>,
    res: grpc.sendUnaryData<GetOneResponse>,
  ) {
    try {
      const request = convertStringToDate(req.request);
