...
import { TQuerySkill, TSkill } from './types';

export class SkillService extends BaseAbstractService<SkillEntity> {
  constructor() {
    super(SkillEntity);
  }

  private readonly entityManager = connection.createEntityManager();
  async getList(query: TQuerySkill) {
  ...
  async create(dto: TSkill, em?: EntityManager) {
    const trx = em ?? this.entityManager;
    try {
      return trx.transaction((trx) => {
        return trx.save(trx.create(SkillEntity, { ...dto }));
      });
    } catch (error) {
  ...
  async update(id: number, dto: TSkill, em?: EntityManager) {
    const trx = em ?? this.entityManager;
    const data = await this.getById(id);
    if (!data) throw new grpcErrorNotFound();
    // const updateData: SkillEntity = Object.assign(data, dto);
    try {
      return trx.transaction((trx) => {
        return trx.save(trx.create(SkillEntity, { ...dto }));
      });
