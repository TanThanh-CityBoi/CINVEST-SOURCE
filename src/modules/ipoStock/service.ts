...
import { TIpoStock, TQueryIpoStock } from './types';
import { relations } from './utils';

export class IpoStockService extends BaseAbstractService<IpoStockEntity> {
  constructor() {
    super(IpoStockEntity);
  }

  private readonly entityManager = connection.createEntityManager();
  async getList(query: TQueryIpoStock) {
  ...
  async create(dto: TIpoStock, em?: EntityManager) {
    const trx = em ?? this.entityManager;
    try {
      return trx.transaction((trx) => {
        return trx.save(trx.create(IpoStockEntity, { ...dto }));
      });
    } catch (error) {
  ...
  async update(id: number, dto: TIpoStock, em?: EntityManager) {
    const trx = em ?? this.entityManager;
    const data = await this.getById(id);
    if (!data) throw new grpcErrorNotFound();
    // const updateData: IpoStockEntity = Object.assign(data, dto);
    try {
      return trx.transaction((trx) => {
        return trx.save(trx.create(IpoStockEntity, { ...dto }));
      });
