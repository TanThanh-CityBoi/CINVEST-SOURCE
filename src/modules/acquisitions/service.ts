...
import { TAcquisition, TQueryAcquisition } from './types';
import { relations } from './utils';

export class AcquisitionService extends BaseAbstractService<AcquisitionEntity> {
  constructor() {
    super(AcquisitionEntity);
  }

  private readonly entityManager = connection.createEntityManager();

  async getList(query: TQueryAcquisition) {
  ...
  async create(dto: TAcquisition, em?: EntityManager) {
    const trx = em ?? this.entityManager;
    try {
      return await trx.transaction(async (trx) => {
        return await trx.save(trx.create(AcquisitionEntity, { ...dto }));
      });
    } catch (error) {
  ...
  async update(id: number, dto: TAcquisition, em?: EntityManager) {
    const trx = em ?? this.entityManager;
    const data = await this.getById(id);
    if (!data) throw new grpcErrorNotFound();
    // const updateData: AcquisitionEntity = Object.assign(data, dto);
    try {
      return trx.transaction((trx) => {
        return trx.save(trx.create(AcquisitionEntity, { ...dto }));
      });
    } catch (error) {
      throw new grpcErrorBadRequest(JSON.stringify(error));
    }
