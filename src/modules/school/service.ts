...
import { TQuerySchool, TSchool } from './types';

export class SchoolService extends BaseAbstractService<SchoolEntity> {
  constructor() {
    super(SchoolEntity);
  }

  private readonly entityManager = connection.createEntityManager();
  async getList(query: TQuerySchool) {
  ...
  async create(dto: TSchool, em?: EntityManager) {
    const trx = em ?? this.entityManager;
    try {
      return trx.transaction((trx) => {
        return trx.save(trx.create(SchoolEntity, { ...dto }));
      });
    } catch (error) {
  ...
  async update(id: number, dto: TSchool, em?: EntityManager) {
    const trx = em ?? this.entityManager;
    const data = await this.getById(id);
 if (!data) throw new grpcErrorNotFound();
    // const updateData: SchoolEntity = Object.assign(data, dto);
    try {
      return trx.transaction((trx) => {
        return trx.save(trx.create(SchoolEntity, { ...dto }));
      });
