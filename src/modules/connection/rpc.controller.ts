...
import { Connection__Output } from '../../proto/types/connections/Connection';
import { QueryConnection__Output } from '../../proto/types/connections/QueryConnection';
...
export class rpcConnectionController {
  private readonly service = new ConnectionService();

  async getList(
    req: grpc.ServerUnaryCall<QueryConnection__Output, GetListResponse>,
  ...
  async create(
    req: grpc.ServerUnaryCall<Connection__Output, GetOneResponse>,
    res: grpc.sendUnaryData<GetOneResponse>,
  ) {
    try {
      const request = convertStringToDate(req.request);
  ...
  async acceptConnection(
    ...
    try {
      const request = convertStringToDate(req.request);
      const data = await this.service.acceptConnection(request.id);
      const result = convertDateToString([data]);
