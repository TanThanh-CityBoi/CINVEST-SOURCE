...
import { TProductType, TQueryProductType } from './types';

export class ProductTypeService extends BaseAbstractService<ProductTypeEntity> {
  constructor() {
    super(ProductTypeEntity);
  }

  private readonly entityManager = connection.createEntityManager();
  async getList(query: TQueryProductType) {
  ...
  async create(dto: TProductType, em?: EntityManager) {
    const trx = em ?? this.entityManager;
    try {
      return trx.transaction((trx) => {
        return trx.save(trx.create(ProductTypeEntity, { ...dto }));
      });
    } catch (error) {
  ...
  async update(id: number, dto: TProductType, em?: EntityManager) {
    const trx = em ?? this.entityManager;
    const data = await this.getById(id);
    if (!data) throw new grpcErrorNotFound();
    // const updateData: ProductTypeEntity = Object.assign(data, dto);
    try {
      return trx.transaction((trx) => {
        return trx.save(trx.create(ProductTypeEntity, { ...dto }));
      });
