...
import { Album__Output } from '../../proto/types/albums/Album';
...
import { QueryAlbum__Output } from '../../proto/types/albums/QueryAlbum';
...
export class rpcAlbumController {
  private readonly service = new AlbumService();

  async getList(
    req: grpc.ServerUnaryCall<QueryAlbum__Output, GetListResponse>,
  ...
  async create(
    req: grpc.ServerUnaryCall<Album__Output, GetOneResponse>,
    res: grpc.sendUnaryData<GetOneResponse>,
  ) {
    try {
      const request = convertStringToDate(req.request);
