...
import { TProduct, TQueryProduct } from './types';
import { relations } from './utils';

export class ProductService extends BaseAbstractService<ProductEntity> {
  constructor() {
    super(ProductEntity);
  }

  private readonly entityManager = connection.createEntityManager();
  async getList(query: TQueryProduct) {
  ...
  async create(dto: TProduct, em?: EntityManager) {
    const trx = em ?? this.entityManager;
    try {
      return trx.transaction((trx) => {
        return trx.save(trx.create(ProductEntity, { ...dto }));
      });
    } catch (error) {
  ...
  async update(id: number, dto: TProduct, em?: EntityManager) {
    const trx = em ?? this.entityManager;
    const data = await this.getById(id);
    if (!data) throw new grpcErrorNotFound();
    // const updateData: ProductEntity = Object.assign(data, dto);
    try {
      return trx.transaction((trx) => {
        return trx.save(trx.create(ProductEntity, { ...dto }));
      });
